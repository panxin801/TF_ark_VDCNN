...
# Update: 2020.3.20 VDCNN has not been modified by now.
...
def downconv(x,
    ...
    w_init = init
    if w_init is None:
        w_init = tf.xavier_initializer(uniform=uniform)
    with tf.variable_scope(name):
        W = tf.get_variable(
            'W', [kwidth, 1, x.get_shape()[-1], output_dim],
            initializer=w_init)
        conv = tf.nn.conv2d(x, W, strides=[1, pool, 1, 1], padding='SAME')
...
def deconv(x,
    ...
    if w_init is None:
        w_init = tf.xavier_initializer(uniform=uniform)
    with tf.variable_scope(name):
        # filter shape: [kwidth, kwidth, output_channels, in_channels]
        W = tf.get_variable(
...
class ANFCN():
    def __init__(self, noise_wave, batchsize, is_ref=True, do_prelu=False):
        kwidth = 5  # 31->5
        skips = []
        # g_enc_depths = [16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 1024]
        g_enc_depths = [16, 32, 32, 64, 64, 128]
        noise_wave_shape = noise_wave.get_shape().as_list()
        self.input_x = tf.placeholder(
            tf.float32, [batchsize, noise_wave_shape[1], 1, 1],
            name="noise_wave")
        self.input_y = tf.placeholder(
            tf.float32, [batchsize, noise_wave_shape[1], 1, 1],
            name="clean_wave")
        ...
        with tf.variable_scope('Encode'):
            ...
            for layer_idx, layer_depth in enumerate(g_dec_depths):
                ...
                out_shape = [h_i_dim[0], ssha[1], 1, layer_depth]
                bias_init = None
                # deconv
                if is_ref:
