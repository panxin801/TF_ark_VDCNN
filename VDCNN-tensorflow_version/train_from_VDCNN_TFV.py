...
# Parameters settings
# Data loading params
tf.flags.DEFINE_string("database_path", "ag_news_csv/",
                       "Path for the dataset to be used.")

# Model Hyperparameters
tf.flags.DEFINE_integer("sequence_max_length", 1014,
...
tf.flags.DEFINE_boolean("optional_shortcut", False,
...
with tf.control_dependencies(update_ops):
    ...
    learning_rate = tf.train.exponential_decay(
        FLAGS.learning_rate,
        global_step,
        FLAGS.num_epochs * num_batches_per_epoch,
        0.95,
        staircase=True)
...
def train_step(x_batch, y_batch):
    ...
    print("{}: Step {}, Epoch {}, Loss {:g}, Acc {:g}".format(
        time_str, step,
...
for train_batch in train_batches:
    ...
    if current_step % FLAGS.evaluate_every == 0:
        ...
        acc_list.append(acc)
        print("{}: Evaluation Summary, Loss {:g}, Acc {:g}".format(
            time_str, sum_loss / i, acc))
        print("{}: Current Max Acc {:g} in Iteration {}".format(
